...
impl ModuleConfig {
    ...
    /// Path of the generated bindings file for `version`.
    fn bindings_path(&self, version: Version) -> PathBuf {
        Path::new(".")
            .join("out")
            .join(self.cudarc_name)
            .join("sys")
            .join("linked")
            .join(format!("sys_{version}.rs"))
    }

    fn bindings_exist(&self, version: Version) -> bool {
    ...
    fn run_bindgen(
        ...
        let outfilename = self.bindings_path(version);
        if outfilename.exists() {
            // Already generated by a previous (possibly partial) run.
            return Ok(());
        }
        let linked_dir = outfilename
            .parent()
            .with_context(|| format!("No parent directory for {}", outfilename.display()))?;
...
fn create_bindings(modules: &[ModuleConfig], cuda_versions: &[Version]) -> Result<()> {
    ...
    while !remaining.is_empty() {
        ...
        let results = ready
            ...
            .map(|(cuda_version, module)| {
                // Resume support: when these bindings already exist and no pending
                // dependent still needs this module's headers, resolve the archive
                // path without downloading anything.
                let headers_needed = !module.bindings_exist(*cuda_version)
                    || modules.iter().any(|m| {
                        m.lib_versions.is_empty()
                            && m.module_dependencies.contains(&module.cudarc_name)
                            && m.supports_cuda_version(*cuda_version)
                            && !m.bindings_exist(*cuda_version)
                    });
                let archive_dir = if headers_needed {
    ...
    lib_tasks
        ...
        .map(|(module, lib_version)| {
            if module.bindings_exist(lib_version) {
                pb.inc(1);
                return Ok(());
            }
...
/// Resolve a module's archive in the redistrib manifest: its relative path,
/// sha256 checksum and local tarball path.
fn resolve_archive(
    cuda_version: &Version,
    cuda_name: &str,
    module_name: &str,
    ...
    let relative_path = lib["relative_path"].as_str().with_context(|| {
        format!("Missing linux-x86_64 relative_path for {cuda_name} in the {cuda_version} redistrib manifest")
    })?;
    let sha256 = lib["sha256"].as_str().with_context(|| {
        format!(
            "Missing linux-x86_64 sha256 for {cuda_name} in the {cuda_version} redistrib manifest"
        )
    })?;
    let file_name = Path::new(relative_path)
        .file_name()
        .with_context(|| format!("No file name in relative_path '{relative_path}'"))?;
    let tarball_path = downloads_dir.join(module_name).join(file_name);
    Ok((relative_path.to_string(), sha256.to_string(), tarball_path))
}
...
fn get_archive(
    ...
    let (relative_path, sha256, tarball_path) = resolve_archive(
        cuda_version,
        cuda_name,
        module_name,
        downloads_dir,
    ...
    download::to_file_with_checksum(
        &format!("{CUDA_REDIST_URL}/{relative_path}"),
        &tarball_path,
        &sha256,
        multi_progress,
    )?;
    extract::extract_archive(&tarball_path, multi_progress)
}

/// The directory `get_archive` would extract to, resolved without downloading
/// the archive itself.
fn expected_archive_dir(
    cuda_version: &Version,
    cuda_name: &str,
    module_name: &str,
    downloads_dir: &Path,
    ...
    let (_, _, tarball_path) = resolve_archive(
        cuda_version,
        cuda_name,
        module_name,
        downloads_dir,
    ...
    // Mirrors extract::extract_archive's output naming.
    let dir = match tarball_path.extension().and_then(|s| s.to_str()) {
        Some("xz") | Some("gz") => tarball_path.with_extension("").with_extension(""),
        _ => tarball_path.with_extension(""),
    };
    Ok(dir)
}
